...
class opParams:
  def __init__(self):
    ...
    self.fork_params = {'awareness_factor': Param(100, VT.number, 'Multiplier for the awareness times'),
                        ...
                        'curvature_factor': Param(999, VT.number, 'Multiplier for the curvature slowdown. Increase for less braking.'),
                        ...
                        'indi_inner_gain_bp': Param([5.5, 8.3, 11.1, 13.9, 16.7, 19.4, 22.2, 25], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'indi_inner_gain_v': Param([4, 5.5, 7.3, 9.1, 10.9, 12.8, 14.5, 15], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'indi_outer_gain_bp': Param([5.5, 8.3, 11.1, 13.9, 16.7, 19.4, 22.2, 25, 30, 36], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'indi_outer_gain_v': Param([4, 4.55, 6.35, 8.15, 9.95, 11.75, 13.55, 15.35, 16, 17], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'indi_time_constant_bp': Param([5.5, 8.3, 11.1, 13.9, 16.7, 19.4, 22.2, 30.09, 30.1, 33.32, 33.33], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'indi_time_constant_v': Param([1, 0.46, 0.57, 0.7, 0.91, 1.25, 3.0, 4.5, 6.5, 6.5, 8.0], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'indi_actuator_effectiveness_bp': Param([5.5, 8.3, 11.1, 13.9, 16.7, 19.4, 22.2, 25], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'indi_actuator_effectiveness_v': Param([15, 15, 15, 15, 15, 15, 15, 15], [list, float, int], live=True, depends_on='enable_indi_live'),
                        'steer_limit_timer': Param(5, VT.number, live=True, depends_on='enable_indi_live'),
                       }
