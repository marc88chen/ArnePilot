...
class Controls:
  ...
  def publish_logs(self, CS, start_time, actuators, v_acc, a_acc, lac_log):
    ...
    if len(meta.desirePrediction) and ldw_allowed:
      ...
      l_lane_change_prob = meta.desirePrediction[Desire.laneChangeLeft - 0]
      r_lane_change_prob = meta.desirePrediction[Desire.laneChangeRight - 0]
      l_lane_close = left_lane_visible and (self.sm['pathPlan'].lPoly[3] < (0.1 - self.dp_camera_offset))
      r_lane_close = right_lane_visible and (self.sm['pathPlan'].rPoly[3] > -(0.4 + self.dp_camera_offset))
